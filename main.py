...
with open(path, newline='', encoding='utf-8') as f:
    ...
    for row in reader:
        if len(row) >= 8:
            rows.append(row[:8:2])  # Take only every second value from tracking data
...
index_mcp_pip_line = cylinder(pos=index_mcp, axis=index_pip - index_mcp, radius=cylinder_radius, color=color.cyan)
middle_mcp_pip_line = cylinder(pos=middle_mcp, axis=middle_pip - middle_mcp, radius=cylinder_radius, color=color.cyan)
ring_mcp_pip_line = cylinder(pos=ring_mcp, axis=ring_pip - ring_mcp, radius=cylinder_radius, color=color.cyan)
pinky_mcp_pip_line = cylinder(pos=pinky_mcp, axis=pinky_pip - pinky_mcp, radius=cylinder_radius, color=color.cyan)
...
# thumb cylinder
thumb_line = cylinder(pos=wrist, axis=thumb - wrist, radius=cylinder_radius, color=color.cyan)
thumb_point = points(pos=thumb)

# loop over the data rows to obtain the finger angles
for i, row in data.iterrows():
    rate(20) # arbitrary for now
    angles = row.values
    ...
    for idx, angle in enumerate(angles):
        ...
        for o, coords in enumerate(finger_coords):
            if o == 0:
                ...
                theta += radians(180-temp_angle)
                coords.y = temp_y + cos(theta)
                coords.z = temp_z + sin(theta)
            elif o == 1:
                temp_angle = 90 + int(angle / 2)
    ...
    # updates the position of the cylinders
    index_mcp_pip_line.axis = index_pip - index_mcp
    middle_mcp_pip_line.axis = middle_pip - middle_mcp
    ring_mcp_pip_line.axis = ring_pip - ring_mcp
    pinky_mcp_pip_line.axis = pinky_pip - pinky_mcp
